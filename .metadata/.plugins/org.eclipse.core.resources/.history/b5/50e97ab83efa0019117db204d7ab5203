...
public class Tabelas {
	...
	protected void Receber(int linha, int coluna) {
		...
		// Verifica se tem número negativo na linha da função objetivo
		for (int j = 0; j < coluna; j++) {
			if (matriz[0][j] < 0) {
				existeNegativo = true;
				break;
		...
		// Enquanto existir um número negativo na primeira linha ele vai seguir
		while (existeNegativo) {
			...
			// Imprime caso o valor pivô seja diferente de 1, nesse caso precisa dividir toda a linha
			...
			// Imprime caso o valor pivô seja igual a 1, nesse caso não precisa dividir toda a linha
			System.out.println();
			System.out.println("------------------ Nova Tabela ------------------");
			for (int i = 0; i < linha; i++) {
				for (int j = 0; j < coluna; j++) {
